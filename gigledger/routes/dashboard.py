...
def index():
    ...
    month_income, month_expenses = calculate_monthly_summary(now.year, now.month)
    balance, tax_obligation, safe_balance = calculate_safe_to_spend(tax_rate)
    runway_balance, avg_monthly_expenses, runway_months = calculate_runway()
    chart_labels, chart_income, chart_expenses = get_6_month_chart_data()
    recent_transactions = get_recent_transactions(limit=5)
    cat_labels, cat_totals = get_category_breakdown()
    ...
    if prev_month == 0:
        prev_month = 12
        prev_year -= 1
    prev_income, prev_expenses = calculate_monthly_summary(prev_year, prev_month)
    ...
    # New: Invoice data
    unpaid_invoices = Invoice.query.filter(Invoice.status.in_(['sent', 'overdue'])).order_by(Invoice.due_date.asc()).all()
